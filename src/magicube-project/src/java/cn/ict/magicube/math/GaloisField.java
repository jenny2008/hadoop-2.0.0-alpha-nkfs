...
public class GaloisField {
	...
	public int[][] reverseVandermonde(int pivot[]) {
		int r = pivot.length;
		...
		for (int i = 0; i < r; i++) {
			...
			/* compute x */
			solveVandermondeSystem(pivot, v);
			for (int j = 0; j < r; j++) {
				result[j][i] = v[j];
			}
		}
	...
	public static void main(String[] args) {
		...
		{	// work
			...
			int[][] vandermondematrix = new int[5][5];
			for (int i = 0; i < 5; i++) {
				for (int j = 0; j < 5; j++) {
					vandermondematrix[j][i] = GF.power(i + 2, j);
				}
			}
			...
			int[][] vand_rev = GF.reverseVandermonde(vandermondv);
			int[][] res = new int[5][5];
			for (int i = 0; i < 5; i++) {
				for (int j = 0; j < 5; j++) {
					res[i][j] = 0;
			...
			for (int i = 0; i < 5; i++) {
				for (int j = 0; j < 5; j++) {
					int v = 0;
					for (int k = 0; k < 5; k++) {
						int xv = GF.mulTable[vand_rev[i][k]][vandermondematrix[k][j]];
						v = GF.add(v, xv);
					}
					res[i][j] = v;					
				}				
